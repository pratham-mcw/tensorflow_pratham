...
TEST(ShiftLeftOpTest, ShiftLeftInt32) {
  ShiftLeftOpModel model({TensorType_INT32, {3}},
                        {TensorType_INT32, {3}});
...
TEST(ShiftLeftOpTest, ShiftLeftInt8) {
  ShiftLeftOpModel model({TensorType_INT8, {2, 2}},
                         {TensorType_INT8,{2, 2}});
  model.PopulateTensor<int8_t>(model.input1(), {2, -2, -2 ,-4});
  model.PopulateTensor<int8_t>(model.input2(), {0, 1, -4, 5}); 
  ASSERT_EQ(model.Invoke(), kTfLiteOk);
  EXPECT_THAT(model.GetOutput<int8_t>(), ElementsAreArray({2, -4, 0, -128}));
}
